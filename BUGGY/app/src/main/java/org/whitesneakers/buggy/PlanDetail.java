...
public class PlanDetail extends AppCompatActivity {
                ...
                for (DataSnapshot child : dataSnapshot.getChildren()) {
                    ...
                    for (int i = 0; i < num; i++) {
                        if (plan.Day.equals(AllDays[i])) {
                            listDataGroup.get(i).getArrayList().add(new PlanDetailChildItems(plan.title, plan.mapX, plan.mapY, plan.message, plan.Day, plan.address));
                        }
                    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode ==7){
            ...
            listDataGroup.get(index).getArrayList().add(new PlanDetailChildItems(spot, x, y, memo, AllDays[index], address));
        }
    }
